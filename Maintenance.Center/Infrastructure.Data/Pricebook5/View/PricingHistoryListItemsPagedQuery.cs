...
namespace Infrastructure.Data.Pricebook5.View
{
    public class PricingHistoryListItemsPagedQuery : IAsyncQuery<IEnumerable<PricingHistoryListItem>, PricingChangeListItemsPagedRequest>
    {
        ...
        {
            string query = "Declare @RecentPriceChanges Table(Id Int Primary Key,Sku Int,Price Decimal(18,3),[Timestamp] Datetime)Insert Into @RecentPriceChanges(Id,Sku,Price,[Timestamp])Select Id,Sku,Price,[Timestamp] From(Select Row_Number()Over(Order By P5H.[Timestamp] Desc,P5H.Sku Asc)As Ordinal,P5H.Id,P5H.Sku,P5H.Price,P5H.[Timestamp] From Pricebook5PricingHistories P5H {PART_INTERNAL_SEARCH})Base Where Ordinal>={PART_RESULT_ORDINAL_MIN} And Ordinal<={PART_RESULT_ORDINAL_MAX} Declare @OldPrices Table(Sku Int Primary Key,Price Decimal(18,3))Insert Into @OldPrices(Sku,Price)Select Sku,Price From(Select Row_Number()Over(Partition By P5H.Sku Order By P5H.[Timestamp] Desc,P5H.Sku Asc)As Ordinal,P5H.Id,P5H.Sku,P5H.Price From Pricebook5PricingHistories P5H Join @RecentPriceChanges B On P5H.Sku=B.Sku Where P5H.Id < B.Id)B Where Ordinal=1 Select PPC.Sku,M.[Description],PPC.Price As CurrentPrice,OP.Price As PreviousPrice,PPC.[Timestamp] As DateChanged From @RecentPriceChanges PPC Left Join @OldPrices OP On PPC.Sku=OP.Sku Join Masterlist M On PPC.Sku=M.Sku Order By PPC.[Timestamp] Desc";
            ...
            query = query.Replace("{PART_INTERNAL_SEARCH}", notHasSearchQuery ?
                    "" :
                    "Where Cast(P5H.Sku As Varchar(15)) Like '%' + @Search + '%' Or Cast(P5H.Store As Varchar(15)) Like '%' + @Search + '%' ")
